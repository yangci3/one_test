  ...
  <div class="app-container">
      ...
      <div v-loading="detailLoading" class="drawer-body">
        <template v-if="detail">
          ...
          <template v-if="manageMode">
            ...
            <p class="section-hint mb16">协作者提交后，内容保存在各人快照中；点击「生成汇总版」后才会合并到「文档管理」中的主文档。</p>
            <el-button
              type="warning"
              plain
              size="small"
    ...
    >
      <div class="submission-meta" v-if="submissionRow">
        <span>协作者：{{ userDisplayName(submissionRow.userId) }}</span>
        <span>状态：{{ submitStatusLabel(submissionRow.submitStatus) }}</span>
      </div>
  ...
  methods: {
    ...
    handleAdd() {
      ...
      Promise.all([this.loadDocOptions(), this.loadCandidates()]).then(() => {
        ...
        this.$modal.msgError('加载创建任务数据失败')
      })
    },
    cancelCreate() {
    ...
    handleViewSubmission(row) {
      ...
      this.submissionHtml = row.contentSnapshot || row.draftContent || '<p>（暂无内容）</p>'
    ...
    navigateToLinkedDoc() {
      ...
      this.$nextTick(() => {
        this.$router.push({
          ...
          this.$modal.msgError('无法打开文档管理页面')
        })
      })
    },
    refreshDetail() {
    ...
    handleMerge() {
      this.$confirm('确认将各协作者提交内容合并到主文档？', '提示', {
        ...
        this.$confirm('是否前往「文档管理」查看合并后的文档？', '汇总完成', {
